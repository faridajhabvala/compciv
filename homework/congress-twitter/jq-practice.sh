echo '1.'
cat data-hold/legislators-current.json | jq '.[0] .name .last' | tr -d '"'
echo '2.'
cat data-hold/legislators-current.json | jq '.[111] .id .bioguide' | tr -d '"'  
echo '3.'
cat data-hold/legislators-current.json | jq '.[13] .terms[2] .start' | tr -d '"'
echo '4.'
cat data-hold/legislators-current.json | jq '.[4] | .name .first, .name .last, .bio .birthday'
echo '5.'
cat data-hold/legislators-current.json | jq '.[] .bio .birthday' | sort -r | head -n 10
echo '6.'
...
echo '17.'
cat data-hold/congress-twitter-profiles.json | jq '.[] .followers_count' | grep -E '[0-9]{7,}'
echo '18.'
...
echo '19.'
cat data-hold/congress-twitter-profiles.json | jq --raw-output '.[] | [ .screen_name, .statuses_count, .followers_count, .verified, .status .created_at] | @csv' | head -n 10
